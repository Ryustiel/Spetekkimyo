...
based on the provided feature file and glyphs. (resources fetched internally for now)
"""
...
def generate_font(output_path: str):
    """
    Run generate.py using the bundled ffpython interpreter.

    Parameters:
        output_path (str): The location of the file to write the font to. (such as "output/font.otf")

    NOTE: generate.py must be run in FontForge's bundled Python environment.
    """
    if output_path[0] in ("/", "\\"):
        raise ValueError("Path must not have / or \\ at position 0.")
